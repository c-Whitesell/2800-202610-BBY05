...
async function logUserTrail(email, body) {
  ...
  const earnedBadges = await checkAndAwardBadges(email);
...
async function checkAndAwardBadges(email) {
  ...
  const newBadges = checkForNewBadges(existingBadges, totalDistance);
...
function checkForNewBadges(existingBadges = [], totalDistance = 0) {
  const thresholds = [
    { id: '10km', distance: 10 },
    { id: '25km', distance: 25 },
    { id: '50km', distance: 50 },
  ...
  return thresholds
    .filter(
      (b) => totalDistance >= b.distance && !existingBadges.includes(b.id),
    )
    .map((b) => b.id);
}
