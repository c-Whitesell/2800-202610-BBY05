...
function wtrNormalise(raw, source) {
  ...
  const distance = parseFloat(
    props.distance || props.length || props.area_ha || 0,
  );
...
function wtrInferTags({
  ...
  if (n.match(/mountain|peak|summit|alpine|ridge|bluff/))
    tags.push("summit", "exposed", "alpine");
...
const WTR_FALLBACK = [
  {
    ...
    lng: -123.021,
    duration: "1.5 hr",
    description: "Dense old-growth canopy.",
    tags: ["forest", "canopy", "valley"],
  },
  {
    _id: "f2",
    ...
    lng: -123.1161,
    duration: "2 hr",
    description: "Flat riverside trail through cedar and fir.",
    tags: ["river", "flat", "canopy"],
  },
  {
    _id: "f3",
    ...
    lng: -123.2042,
    duration: "1.5 hr",
    description: "Urban forest with sheltered trails.",
    tags: ["canopy", "urban", "flat"],
  },
  {
    _id: "f4",
    ...
    lng: -122.9195,
    duration: "2 hr",
    description: "Forested trail to the summit.",
    tags: ["exposed", "summit", "forest"],
  },
  {
    _id: "f5",
    ...
    lng: -122.9503,
    duration: "1.5 hr",
    description: "Dense canopy, open viewpoint at the top.",
    tags: ["forest", "canopy", "viewpoint"],
  },
  {
    _id: "f6",
    name: "Garibaldi Panorama",
    difficulty: "hard",
    distance: 30,
    elevation: 1200,
    location: "Squamish",
    ...
    lng: -122.9908,
    duration: "8 hr",
    description: "Alpine route — dangerous in bad weather.",
    tags: ["alpine", "exposed", "summit"],
  },
];
...
function scoreTrailForWeather(trail, wx) {
  ...
  if (isRaining) {
    if (
      ...
      score += 18;
      reasons.push("Dense canopy keeps you dry");
    } else if (tags.some((t) => ["exposed", "alpine", "open"].includes(t))) {
      score -= 30;
      warnings.push("Exposed trail in rain");
    } else {
      score -= 10;
    }
    if (tags.some((t) => ["marsh", "river", "wetland"].includes(t))) {
      score -= 8;
      warnings.push("Muddy conditions likely");
    }
  }
  ...
  if (isStorming) {
    if (tags.some((t) => ["alpine", "summit", "exposed"].includes(t))) {
      score -= 50;
      warnings.push("Dangerous in storm");
    } else if (tags.some((t) => ["canopy", "forest", "shelter"].includes(t))) {
      score -= 5;
      warnings.push("Check lightning risk near tall trees");
    } else {
      score -= 25;
    }
  }
  ...
  if (isSnowing) {
    if (trail.elevation > 300) {
      score -= 35;
      warnings.push("High elevation – snow/ice likely");
    }
    if (trail.elevation > 600) {
      score -= 30;
      warnings.push("Alpine conditions");
    }
    if (trail.elevation <= 100 && tags.includes("flat")) {
      score += 8;
      reasons.push("Low elevation stays accessible");
    }
  }
  ...
  if (!isRaining && !isStorming) {
    if (uvIndex >= 8) {
      if (tags.some((t) => ["canopy", "forest", "shelter"].includes(t))) {
        score += 20;
        reasons.push("Great shade for high UV");
      } else if (tags.some((t) => ["exposed", "open"].includes(t))) {
        score -= 22;
        warnings.push("High UV, minimal shade");
      }
    } else if (uvIndex >= 4) {
      if (tags.some((t) => ["canopy", "forest"].includes(t))) {
        ...
        score -= 8;
        warnings.push("Limited shade midday");
      }
    } else if (uvIndex <= 2) {
      score += 5;
      if (tags.some((t) => ["summit", "viewpoint"].includes(t)))
  ...
  if (apparentC < 0) {
    ...
    score -= 5;
    warnings.push("Dress in warm layers");
  } else if (apparentC >= 15 && apparentC <= 22) {
    score += 8;
    reasons.push("Ideal hiking temperature");
  } else if (apparentC > 28) {
    if (tags.some((t) => ["canopy", "forest", "valley"].includes(t))) {
      score += 12;
      reasons.push("Canopy keeps it cool");
    } else {
      score -= 18;
      warnings.push("Heat exposure risk");
    }
  }
  ...
  if (windKmh > 50) {
    if (tags.some((t) => ["alpine", "summit", "exposed"].includes(t))) {
      score -= 35;
      warnings.push("Dangerous wind on exposed ridge");
    } else if (tags.some((t) => ["forest", "valley", "canopy"].includes(t))) {
      score += 5;
      reasons.push("Forest shelters from wind");
    } else {
      score -= 15;
      warnings.push("Strong winds expected");
    }
  } else if (windKmh > 25) {
    if (tags.some((t) => ["exposed", "alpine"].includes(t))) {
      score -= 10;
      warnings.push("Gusty at exposed sections");
    }
    if (tags.some((t) => ["forest", "valley"].includes(t))) {
      score += 3;
      reasons.push("Forest windbreak");
    }
  ...
  if (score >= 78) {
    badge = "Perfect today";
    badgeClass = "rec-badge--perfect";
  } else if (score >= 58) {
    badge = "Good choice";
    badgeClass = "rec-badge--good";
  } else if (score >= 38) {
    badge = "Use caution";
    badgeClass = "rec-badge--caution";
  } else {
    badge = "Not recommended";
    badgeClass = "rec-badge--avoid";
  }
...
function getWeatherContextHeadline(wx) {
  ...
  if (wx.isSnowing)
    return { icon: "❄️", text: "Snow conditions — low elevation recommended" };
  if (wx.isRaining)
    return { icon: "🌧️", text: "Rainy day — canopy trails ranked first" };
  if (wx.uvIndex >= 8)
    return { icon: "☀️", text: "High UV — shade is your best friend" };
  if (wx.uvIndex >= 5)
    return { icon: "🌤️", text: "Moderate sun — canopy trails favoured" };
  if (wx.windKmh > 40)
    return { icon: "💨", text: "Windy — valley & forest trails recommended" };
  if (wx.apparentC >= 15 && wx.apparentC <= 22)
    return { icon: "🌿", text: "Ideal conditions — most trails open" };
  if (wx.apparentC < 2)
    return { icon: "🥶", text: "Cold out — bundle up, pick shorter trails" };
...
function renderRecCard(s, rank) {
  ...
  const diffColor =
    diffColors[(trail.difficulty || "").toLowerCase()] || "#888780";
  const scoreBar =
    score >= 78
      ? "#3b6d11"
      : score >= 58
        ...
        : score >= 38
          ? "#ba7517"
          : "#c0392b";
  const distStr = trail.distance
    ? `${parseFloat(trail.distance).toFixed(2)} km`
    : "";
      ...
      <div class="wtr-card__body">
        ...
        ${
          ...
          <div class="wtr-card__warnings">
            ${warnings
              .slice(0, 2)
              .map((w) => `<span class="wtr-warning">⚠ ${w}</span>`)
              .join("")}
          </div>`
            : ""
        }
