...
function normalisePark(raw) {
    ...
    } else if (raw.geometry?.type === 'Polygon' && raw.geometry.coordinates?.[0]?.length) {
...
function filterItems(query) {
    ...
    return allItems.filter(item => {
        ...
        const nameMatch = !q || item.name.toLowerCase().includes(q) || item.location.toLowerCase().includes(q);
...
function renderHistory() {
    ...
    historyList.innerHTML = h.map(term => `
